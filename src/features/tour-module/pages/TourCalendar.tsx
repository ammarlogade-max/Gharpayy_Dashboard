...
export default function TourCalendar() {
  ...
  const tcms = useMemo(() => Array.from(new Set(tours.map((t) => t.tcm_name).filter(Boolean) as string[])), [tours]);
  const days = useMemo(getWeekDays, []);

  const filtered = useMemo(
    () =>
      tours.filter((t) => {
        if (tcmFilter !== "all" && t.tcm_name !== tcmFilter) return false;
  ...
  const intentColor = (intent: string | null) =>
    intent === "hard"
      ? "bg-role-tcm/30 border-role-tcm hover:bg-role-tcm/50"
      : intent === "medium"
      ? "bg-role-hr/30 border-role-hr hover:bg-role-hr/50"
  ...
  return (
    <div className="space-y-4 animate-slide-up">
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <select value={tcmFilter} onChange={(e) => setTcmFilter(e.target.value)} className="h-9 bg-surface-2 border border-border rounded-md px-3 text-sm text-foreground">
      ...
      {!isLoading && error && <div className="glass-card p-4 text-sm text-destructive">{error}</div>}
      {!isLoading && !error && filtered.length === 0 && <div className="glass-card p-8 text-center text-sm text-muted-foreground">No tours found.</div>}
      ...
      {!isLoading && !error && filtered.length > 0 && (
        <>
                      ...
                      <div key={`${d.iso}${h}`} className="border-b border-r border-border/30 p-1 min-h-[40px] space-y-0.5">
                        {items.map((t) => (
                          <div
                            ...
                            className={cn("text-[10px] px-1.5 py-1 rounded border cursor-pointer transition-colors flex items-center gap-1", intentColor(t.intent))}
                          >
                            {tourIcon(t.tour_type)}
                            <span className="truncate text-foreground font-medium">{(t.name || "-").split(" ")[0]}</span>
                          </div>
                        ))}
                      </div>
                    );
          ...
          {hoveredTour && (
            <div className={cn("hidden md:block fixed bottom-4 right-4 z-50 glass-card p-3 max-w-xs border", intentColor(hoveredTour.intent))}>
              <div className="text-sm font-semibold text-foreground">{hoveredTour.name || "-"}</div>
              <div className="text-xs text-muted-foreground">
                {hoveredTour.property || "-"} · {hoveredTour.time || "-"}
              </div>
              <div className="text-xs mt-1">{hoveredTour.live_score ?? hoveredTour.score ?? 0}%</div>
            </div>
          )}

          <div className="md:hidden space-y-3">
            {days.map((d) => {
              const dayTours = filtered.filter((t) => t.date === d.iso).sort((a, b) => (a.time || "").localeCompare(b.time || ""));
              return (
                    ...
                    <div className="space-y-1.5">
                      {dayTours.map((t) => (
                        <div key={t.id} className={cn("flex items-center gap-2 px-2 py-1.5 rounded border", intentColor(t.intent))}>
                          <span className="text-[10px] font-mono text-foreground tabular-nums w-12">{t.time || "-"}</span>
                          {tourIcon(t.tour_type)}
                          <span className="text-xs font-medium text-foreground truncate flex-1">{t.name || "-"}</span>
                          <span className="text-[10px] text-muted-foreground tabular-nums">{t.live_score ?? t.score ?? 0}%</span>
                        </div>
                      ))}
                    </div>
                  )}
