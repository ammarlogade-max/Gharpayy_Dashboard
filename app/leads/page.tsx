...
const Leads = () => {
  ...
  const handleExport = () => {
    const csv = [
      ...
      ...filtered.map(l => [l.name, l.phone, l.email || '', l.source, l.status, l.agents?.name || '', l.preferredLocation || '', l.budget || '', l.leadScore ?? 0].join(','))
    ].join('\n');
              ...
              {filtered.map(lead => (
                <tr key={lead.id} className="border-b border-border last:border-0 hover:bg-secondary/20 transition-colors cursor-pointer group"
                  ...
                  <td className="px-4 py-3.5">
                    <span className={`text-2xs font-semibold flex items-center gap-1 ${scoreColor(lead.leadScore ?? 0)}`}>
