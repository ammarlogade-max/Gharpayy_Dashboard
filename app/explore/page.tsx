      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-14">
            ...
            <Button variant="outline" size="sm" onClick={() => router.push('/auth')}>Login</Button>
          </div>
        </div>
      </header>
        ...
        {viewMode === 'map' && (
          <div className="rounded-xl overflow-hidden border border-border" style={{ height: 'calc(100vh - 200px)' }}>
            <Suspense fallback={<div className="w-full h-full bg-muted flex items-center justify-center text-muted-foreground">Loading map...</div>}>
              <PropertyMap properties={mapProperties} onPropertyClick={(id) => router.push(`/property/${id}`)} />
            </Suspense>
          </div>
        )}
...
export default function Explore() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">Loading explore...</div>}>
      <ExploreContent />
    </Suspense>
  );
}
