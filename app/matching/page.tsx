      ...
      <SelectContent>
        {activeLeads.map(l => (
          <SelectItem key={l.id} value={l.id}>
            {l.name} — {l.preferredLocation || 'No location'} · {l.budget || 'No budget'}
          </SelectItem>
        ))}
      </SelectContent>
    </Select>
...
{lead && (
  <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-xl border bg-card">
    ...
    <div className="flex flex-wrap gap-3 mt-2 text-xs text-muted-foreground">
      {lead.preferredLocation && <span className="flex items-center gap-1"><MapPin size={12} /> {lead.preferredLocation}</span>}
