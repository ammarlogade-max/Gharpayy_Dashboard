              ...
              <tbody>
                {filtered.map(l => (
                  <tr key={l.id} className="border-b border-border hover:bg-secondary/30 transition-colors">
                    ...
                    <td className="px-3 py-2.5 text-muted-foreground">{l.preferredLocation || '—'}</td>
                    <td className="px-3 py-2.5 text-muted-foreground">{format(new Date(l.createdAt), 'MMM d, yyyy')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
